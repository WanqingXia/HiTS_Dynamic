...
class Session:
    ...
    def step_env_and_graph(self, env_info, sess_info, render, render_frequency, episode, total_step_multiplier=1.):
        ...
        # step environment
        new_raw_obs, env_rew, env_done, _, info = self.env.step(action)
        ...
        time = self.graph.active_node.show_info()[0].action['delta_t_ach']
        subgoal = self.graph.active_node.show_info()[0].action['goal']
        self.env.task.set_subgoal(subgoal)
        # print(subgoal)
    ...
    def run(self, n_steps=None, n_episodes=None, max_runtime=None, learn=True, render=False,
            ...
            cb_after_test_episode=None, total_step_init=0, append_to_logfiles=False,
        ...
        assert {type(n_steps), type(n_episodes)} == {int, int}, \
        ...
        # obs = self.env.reset()
        env_info = EnvInfo(
            new_obs=self._process_env_observation(obs),
            done=True)
        sess_info = SessInfo(
            ep_step=0,
        ...
        while (n_steps is None or sess_info.total_step < n_steps) and \
            ...
            if not train or env_info.done or graph_done:
                ...
                if test and episode % test_frequency == 0:
                    ...
                    for test_episode in range(test_episodes):
                        ...
                        while True:
                            env_info, graph_done = self.step_env_and_graph(env_info, sess_info_test,
                                                                           test_render, test_render_frequency,
                                                                           test_episode, 1. / test_episodes)
                    ...
                    if cb_after_test_episode is not None:
                        cb_after_test_episode(self.graph, episode, avg_return_test, avg_length_test, avg_success_test)
