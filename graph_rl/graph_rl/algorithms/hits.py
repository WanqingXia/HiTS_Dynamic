    ...
    def _sample_achieved_timed_goals(self, current_index, episode_start):
        ...
        for i in indices:
            ...
            goals.append({
                ...
                "delta_t_ach_obs": [self._convert_time(
                    self._episode_transitions[i].subtask_tr.info["t"] - (current_t - 1) - rand_shift)],
                "delta_t_ach_new_obs": [
                    self._convert_time(self._episode_transitions[i].subtask_tr.info["t"] - current_t - rand_shift)]
            })
        return goals
