...
import json
import pprint
from graph_rl.graph_rl import Session
...
def run_session(dir, graph, env, run_params, total_step_init=0, callback=None):
    ...
    if "test_frequency" in run_params:
        class Cb_after_test_episode:
            def __init__(self):
                self.last_model_save = 0

            # callback is executed after each training episode
            def __call__(self, graph, episode, avg_return_test, avg_length_test, avg_success_test):

                self.last_model_save = episode
                # save model params
                episode_in_k = int(episode / 1000)
                save_path = os.path.join(save_directory, f"params_{episode_in_k}k.pt")
                graph.save_parameters(save_path)
                # external part of callback
                if callback is not None:
                    callback(graph, avg_return_test, avg_length_test, avg_success_test)

        cb_after_test_episode = Cb_after_test_episode()
    else:
        cb_after_test_episode = None

    tensorboard_log = False if "tensorboard_log" not in run_params else run_params["tensorboard_log"]

    sess_props = sess.run(
        n_steps=run_params["n_steps"],
        n_episodes=run_params["n_episodes"],
