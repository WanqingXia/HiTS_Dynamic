...
from graph_rl.graph_rl.graphs import HiTSGraph, HACGraph
...
def create_graph_hits(env, n_layers, n_steps, subtask_specs, level_algo_kwargs_list):
    ...
    # case in general).
    # This requires the env to have an attribute max_episode_length.
    ...
    for i in range(n_layers):
        n_steps = n_steps/100  # reducing buffer size to avoid memory issues
        if "buffer_size" not in level_algo_kwargs_list[i]:
            n_goals = level_algo_kwargs_list[i]["n_hindsight_goals"] + 1
            bs_factor = level_algo_kwargs_list[i]["buffer_size_factor"] if "buffer_size_factor" in \
                                                                           level_algo_kwargs_list[i] else 1.0
            if i == n_layers - 1:
                level_algo_kwargs_list[i]["buffer_size"] = int(bs_factor *
                                                               n_steps / env.max_episode_length * subtask_specs[
                                                                   i]._max_n_actions) * n_goals
            else:
    ...
    for i in range(n_layers):
        if "buffer_size" not in level_algo_kwargs_list[i]:
            ...
            bs_factor = level_algo_kwargs_list[i]["buffer_size_factor"] if "buffer_size_factor" in \
                                                                           level_algo_kwargs_list[i] else 1.0
            level_algo_kwargs_list[i]["buffer_size"] = int(bs_factor
                                                           * n_steps / env.max_episode_length * np.prod(
                max_n_actions_list[i:])) * n_goals
            print(f"Buffer size level {i}: {level_algo_kwargs_list[i]['buffer_size']}")
            if "buffer_size_factor" in level_algo_kwargs_list[i]:
                del level_algo_kwargs_list[i]["buffer_size_factor"]
